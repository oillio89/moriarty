...
## Upgrading translation quality (DeepL via Supabase Edge Function)

MyMemory (the default translator) is free and needs no setup, but its
quality can be rough on longer news sentences. DeepL is much better — but
DeepL **blocks all direct browser requests**, by design, to protect API
keys. The fix: run a tiny serverless function on Supabase (which you
already have) that calls DeepL on the server side, and have the app call
that function instead.

The function file is already written for you, at
`supabase/functions/translate/index.ts`. Moriarty's code already tries this
function first and silently falls back to MyMemory if it isn't deployed —
so this whole section is optional, and nothing breaks either way.

### 1. Get a free DeepL API key

1. Go to [deepl.com/pro-api](https://www.deepl.com/pro-api) and sign up for
   the **API Free** plan (500,000 characters/month free; a card is required
   for verification but you won't be charged unless you exceed the free
   tier and upgrade).
2. Copy your API key from the account page. Free-tier keys end in `:fx`.
...
```bash
supabase login
supabase link --project-ref prylzmsbcebtldmbeygm
```

(Your project ref is the subdomain in your Supabase URL —
`https://prylzmsbcebtldmbeygm.supabase.co` → ref is `prylzmsbcebtldmbeygm`.)

### 4. Set your DeepL key as a secret

```bash
supabase secrets set DEEPL_API_KEY=your_deepl_key_here
```

This keeps the key on Supabase's servers — it's never sent to the browser.

### 5. Deploy the function

From the folder containing the `supabase/` directory:

```bash
supabase functions deploy translate --no-verify-jwt
```

`--no-verify-jwt` lets the app call it with just the public anon key, which
is what Moriarty's code already does — no extra auth wiring needed.

### 6. Test it

Reload the deployed app, go to News, tap a live headline's "แปลไทย" button.
If the function is working, translations should read noticeably more
naturally. If something's off, check the function logs:

```bash
supabase functions logs translate
```

If the function fails for any reason (not deployed yet, DeepL quota hit,
etc.), the app automatically falls back to MyMemory — you'll never see a
broken translation button, just a lower-quality result.
